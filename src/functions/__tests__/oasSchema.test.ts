...
import { createWithRules } from "../../../__tests__/__helpers__/helper";
import { RulesetFunctionContext } from "@stoplight/spectral-core";

function runSchema(
  target: unknown,
  schemaObj: Record<string, unknown>,
  context?: DeepPartial<RulesetFunctionContext>
) {
...
describe("oasSchema", () => {
  ...
  test("should remove all redundant ajv errors", async () => {
    const spectral = createWithRules([
      "oas3-schema",
      "oas3-valid-schema-example",
      "oas3-valid-media-example",
    ]);
    const invalidSchema = JSON.stringify(
      require("../../../__tests__/__fixtures__/petstore.invalid-schema.oas3.json")
    );
